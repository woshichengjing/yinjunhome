/* Hangzhou forecast. Public weather data only; independent of device controls. */
(function () {
  'use strict';
  var panel = document.getElementById('weather-forecast');
  if (!panel) return;
  var zone = 'Asia/Shanghai';
  var cacheKey = 'hangzhou-forecast-30.31-120.12-v1';
  var refreshMs = 30 * 60 * 1000;
  var url = 'https://api.open-meteo.com/v1/forecast?latitude=30.31&longitude=120.12' +
  ...
  function valid(data) {
    return data && data.hourly && data.daily && Array.isArray(data.hourly.time) &&
      ...
      ['temperature_2m', 'weather_code', 'precipitation_probability', 'is_day'].every(function (key) {
        return Array.isArray(data.hourly[key]) && data.hourly[key].length === data.hourly.time.length;
      }) && ['temperature_2m_min', 'temperature_2m_max', 'weather_code', 'precipitation_probability_max'].every(function (key) {
  ...
  function icon(code, day) {
    ...
    var rain = number(code) && code >= 51;
    svg.setAttribute('viewBox', '0 0 24 24');
    svg.setAttribute('fill', 'none'); svg.setAttribute('stroke', 'currentColor');
    svg.setAttribute('stroke-width', '1.5'); svg.setAttribute('stroke-linecap', 'round');
    svg.setAttribute('class', 'forecast-icon ' + (rain ? 'rain' : code > 1 ? 'cloud' : ''));
    svg.setAttribute('role', 'img'); svg.setAttribute('aria-label', codes[code] || '天气未知');
    if (!number(code)) svg.innerHTML = '<path d="M9 8a3 3 0 1 1 5 2c-2 1-2 2-2 3m0 4v.1"/>';
    else if (code <= 1) svg.innerHTML = day === 0 ? '<path d="M20 15A9 9 0 0 1 9 4a8 8 0 1 0 11 11Z"/>' : sun;
    else if (code >= 95) svg.innerHTML = cloud + '<path d="m12 13-3 5h4l-2 4"/>';
    else if ((code >= 71 && code <= 77) || code === 85 || code === 86) svg.innerHTML = cloud + '<path d="M8 20h.1m5 1h.1m5-1h.1"/>';
    else if (rain) svg.innerHTML = cloud + '<path d="m8 19-1 3m6-3-1 3m6-3-1 3"/>';
    else svg.innerHTML = cloud + (code >= 45 ? '<path d="M4 20h16"/>' : '');
    return svg;
  }
  ...
  var footer = node('div', 'forecast-footer');
  var source = node('a', '', 'Open-Meteo'); source.href = 'https://open-meteo.com/'; source.target = '_blank'; source.rel = 'noopener noreferrer';
  var stamp = node('span', '', '正在更新'); footer.append(source, stamp);
  ...
  function hourly() {
    ...
    return h.time.map(function (time, i) {
      return {time: new Date(time + '+08:00'), temp: h.temperature_2m[i], rain: h.precipitation_probability[i], code: h.weather_code[i], day: h.is_day[i]};
  ...
  function renderHourly() {
    ...
    var summary = node('p', 'forecast-summary', '未来' + items.length + '小时  ');
    summary.appendChild(node('strong', '', degrees(low) + ' — ' + degrees(high)));
    ...
    items.forEach(function (item, i) {
      ...
      label.title = localDate(item.time) + ' ' + localTime(item.time) + ' · ' + (codes[item.code] || '未知');
      label.appendChild(icon(item.code, item.day)); labels.appendChild(label);
      rain.appendChild(node('span', '', probability(item.rain)));
      if (!number(item.temp)) { if (path.length) paths.push(path); path = []; return; }
      var x = i * 50 + 25, y = 64 - (item.temp - low) / span * 32;
      path.push(x + ',' + y); points.push({x:x, y:y, temp:item.temp});
    });
    if (path.length) paths.push(path);
    var svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
    svg.setAttribute('class', 'forecast-curve'); svg.setAttribute('width', width); svg.setAttribute('viewBox', '0 0 ' + width + ' 88');
    svg.setAttribute('role', 'img'); svg.setAttribute('aria-label', '逐小时室外气温曲线，单位摄氏度');
    svg.innerHTML = '<path d="M0 78H' + width + '" stroke="#a5c5e012"/>' +
    ...
    var legend = node('div', 'forecast-legend'); legend.append(node('span', '', '气温 °C'), node('span', '', '下方为降雨概率'));
  ...
  function renderDaily() {
    ...
    content.appendChild(node('p', 'forecast-summary', '每日天气 · 最低 / 最高气温'));
    ...
    indexes.forEach(function (i) {
      ...
      var condition = node('span', 'forecast-condition', codes[d.weather_code[i]] || '未知');
      ...
      row.append(node('span', 'forecast-day-name', label), icon(d.weather_code[i], 1), condition, node('span', 'forecast-low', degrees(low)), range, node('span', 'forecast-high', degrees(high)));
      list.appendChild(row);
    });
    content.appendChild(list);
    content.appendChild(node('div', 'forecast-legend', '百分比为当日最高降雨概率'));
  }
  function render() {
    ...
    stamp.textContent = updated ? (failed ? '缓存 · ' : '') + localTime(new Date(updated)) + ' 更新' : loading ? '正在更新' : '未连接';
    stamp.title = updated ? localDate(new Date(updated)) + ' ' + localTime(new Date(updated)) + '（北京时间）' : '';
  }
  async function load(force) {
    ...
    try {
      var response = await fetch(url, {signal:abort.signal, credentials:'omit', referrerPolicy:'no-referrer'});
      ...
      snapshot = data; updated = Date.now(); failed = false;
