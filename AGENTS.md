...
- **自动温控只管理制冷/除湿，不自动制热**。设备处于 `heat` 时视为用户手动模式，引擎不改模式、温度或风速。空调原则上不自动开关机，仅两个例外：① 连续节能待机 → 关机；② AC 全关、房间有人且明确判为偏热/过热/过湿，并且不在新风优先观察期 → 自动以制冷模式开一台。偏冷/过冷拥有否决权；偏湿先交给新风或独立除湿机，只有过湿才允许 AC dry。房间舒适时保持关机。常规低风降噪，仅过热才高风。
...
- **新风触发与优先级**：室外 20–26°C 且室外绝对湿度 < 室内最低，或 CO₂ > 1000 强制。房间偏热且室外至少低 2°C 时先开启新风并观察 30 分钟；观察期内空调保持关闭，之后仍不舒适才允许自动开机。新风未确认启动时最多等待 2 分钟，避免设备故障时无限阻止空调兜底。
